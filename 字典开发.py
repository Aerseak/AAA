import random
print("==================")
print("|   中国工商银行|   ")
print("==================")
print("|1、开户          |")
print("|2、存钱          |")
print("|3、取钱          |")
print("|4、转账          |")
print("|5、查询          |")
print("|6、退出          |")
print("==================")

bank={
1: {'account': 1, 'username': '卢嘉', 'password': '123456', 'country': '中国', 'province': '广东', 'street': '揭阳路', 'door': '369', 'money': 1000},
2: {'account': 2, 'username': 'Aerseak', 'password': '123456', 'country': '中国', 'province': '广东', 'street': '揭阳路', 'door': '369', 'money': 1000}
}

def useradd():
    account=random.randint(1000000000,9999999999)
    username=input("请输入您的姓名：")
    password=input("请输入您的密码：")
    country=input("\t\t请输入您的国家：")
    province=input("\t\t请输入您的省份：")
    street=input("\t\t请输入您的街道：")
    door=input("\t\t请输入您的门牌号：")
    money=0
    gift=bankadd(account,username,password,country,province,street,door,money)
    if gift == "1":
        print("开户成功")
        info = '''
                    ------工商银行------
                    1、账号：%s
                    2、姓名：%s
                    3、密码：******
                    4、国家：%s
                    5、省份：%s
                    6、街道：%s
                    7、门牌：%s
                    8、余额：%s
        '''
        print(info % (account,username,country,province,street,door,money))
        print(bank)
    elif gift == "2":
        print("该账户名已被占用，请输入其他的账户名！")
    elif gift == "3":
        print("该数据库已满，请联系客服进行维修，给您带来不便，非常抱歉！")

def bankadd(account,username,password,country,province,street,door,money):
    if username in bank:
        return "2"
    elif len(bank) >= 100:
        return "3"
    bank[account]={
        "account":account,
        "username": username,
        "password": password,
        "country": country,
        "province": province,
        "street": street,
        "door": door,
        "money":money
    }
    return "1"


#存钱
def accessadd():
    account = input("请输入帐户：")
    account = int(account)
    cq = input("请输入存款金额：")
    cq = int(cq)
    ba = bank_access(account,cq)
    if ba == True:
        print("存入成功")
        print("当前帐户余额：",bank[account]['money'])
    else:
        print("存钱失败！")

def bank_access(account,cq):
    if account in bank:
        bank[account]['money'] += cq
        return True
    else:
        return False

#取钱
def moneyreduce():
    account = input("请输入帐户：")
    account = int(account)
    qc = input("请输入提取金额：")
    qc = int(qc)
    password = input("请输入密码：")
    br = bank_reduce(account,qc,password)
    if br == 0:
        print("取钱成功")
        print("当前帐户余额：",bank[account]['money'])
    elif br == 1:
        print("账号不存在")
    elif br == 2:
        print("密码错误")
    elif br == 3:
        print("钱不够")

def bank_reduce(account,qc,password):
    if account in bank:
        if password == bank[account]["password"]:
            if qc <= bank[account]['money']:
                bank[account]['money'] -= qc
                return 0
            else:
                return 3
        else:
            return 2
    else:
        return 1

#转账
def zhuanzhang():
    account = input("请转出输入帐号：")
    account = int(account)
    accountB = input("请输入转入帐号：")
    accountB = int(accountB)
    je = input("请输入转出金额：")
    je = int(je)
    password = input("\t请输入密码：")
    bs = bank_show(account,accountB,je,password)
    if bs == 0:
        print("转账成功！")
        print("转出帐户",account,"现有金额：",bank[account]["money"])
        print("转入帐户",accountB,"现有金额",bank[accountB]["money"])
    elif bs == 1:
        print("转出或转入帐户不存在，请仔细检查输入的帐号")
    elif bs == 2:
        print("密码错误！！！")
    elif bs == 3:
        print("转出帐户的余额不足！")

def bank_show(account,accountB,je,password):
    if account in bank and accountB in bank:
        if password == bank[account]["password"]:
            if je <= bank[account]['money']:
                bank[account]["money"] -= je
                bank[accountB]["money"] += je
                return 0
            else:
                return 3
        else:
            return 2
    else:
        return 1

#查询
def i():
    account = input("请输入您的帐号：")
    account = int(account)
    password = input("请输入您的密码：")
    o = bank_i(account,password)
    if o == 0:
        print(bank[account])
    elif o == 1:
        print("帐号错误！")
    elif o == 2:
        print("密码错误！")

def bank_i(account,password):
    if account in bank:
        if password == bank[account]["password"]:
            return 0
        else:
            return 2
    else:
        return 1


while True:
    box=input("请输入业务的编号：")
    if box == "1":
        print("1、开户")
        useradd()
    elif box == "2":
        print("2、存钱")
        accessadd()
    elif box == "3":
        print("3、取钱")
        moneyreduce()
    elif box == "4":
        print("4、转账")
        zhuanzhang()
    elif box == "5":
        print("5、查询")
        i()
    elif box == "6":
        print("Buy!Buy!")
        break